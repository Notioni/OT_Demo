...
/* in second */
#define AUDIO_MAX_TIME            4

#ifdef FLASH_MONO_DATA
#define AUDIO_MAX_SIZE            (AUDIO_MAX_TIME * 16000 * SAI_DATA_BYTES)
#else
#define AUDIO_MAX_SIZE            (AUDIO_MAX_TIME * 16000 * SAI_DATA_BYTES * 2)
#endif
...
static int reinit_sai_and_dma(SAI_DIRECTION dir)
{
	...
	switch (dir) {
		case SAI_dir_tx_m2p:
			hal_gpio_output_high(&brd_gpio_table[GPIO_GS_LED]); //for old hw ver
			break;
		case SAI_dir_rx_p2m:
			hal_gpio_output_low(&brd_gpio_table[GPIO_GS_LED]);  //for old hw ver
			break;
		default:
	...
	hsai_BlockA2.Init.AudioMode = (dir == SAI_dir_tx_m2p ? SAI_MODEMASTER_TX : SAI_MODEMASTER_RX);
	hsai_BlockA2.Init.ClockStrobing = (dir == SAI_dir_tx_m2p ? SAI_CLOCKSTROBING_FALLINGEDGE : SAI_CLOCKSTROBING_RISINGEDGE);
	...
	hdma_sai2_a.Init.Direction = (dir == SAI_dir_tx_m2p ? DMA_MEMORY_TO_PERIPH : DMA_PERIPH_TO_MEMORY);
...
int record_to_flash(void)
{
	...
	ret = HAL_SAI_Receive_DMA(&hsai_BlockA2, (uint8_t *)data_buff, DATA_BUFF_LEN);
	if (ret != 0)	{
	...
	while (1) {
		...
		if ((pos_flash + part_bytes / 2) > AUDIO_MAX_SIZE) {
			ret = HAL_SAI_DMAStop(&hsai_BlockA2);
			if (ret != 0) {
...
int playback_from_flash(void)
{
	...
	ret = HAL_SAI_Transmit_DMA(&hsai_BlockA2, (uint8_t *)data_buff, DATA_BUFF_LEN);
	if (ret != 0) {
